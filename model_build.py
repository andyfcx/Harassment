...
def preprocessing():
    ...
    df_parsed = df.drop(columns=['主文', '犯罪事實', '理由', '案例', '案由', '法院', '案號', 'label_', '備註'],
                        axis=1)
    df_parsed.to_csv("data/follow_matrix.csv", index=False)
    return df_parsed


df_parsed = preprocessing()
x_train, x_test, y_train, y_test = train_test_split(df_parsed[['judge', 'facts', 'reason', 'accused', 'annotation']],
